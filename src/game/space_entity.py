...
class SpaceEntity:
    ...
    def _dibujar_nave_roja(self, img, escala=1.0):
        ...
        # --- Efecto de humo cuando la nave es pequeña (al fondo) ---
        if escala < 0.7:
            for i in range(3):
                alpha = 0.18 - i * 0.05
                smoke = img.copy()
                cv2.circle(smoke, (cx, cy + int(body_h * 0.7)), int(body_w * (0.25 + 0.12 * i)), (120, 120, 120), -1, cv2.LINE_AA)
        ...
        for i in range(body_h):
            ...
            cv2.ellipse(img, (cx, cy), (body_w // 2, body_h // 2 - i // 2), 0, 0, 360, color, 1, cv2.LINE_AA)
        cv2.ellipse(img, (cx, cy), (body_w // 2, body_h // 2), 0, 0, 360, (40, 40, 200), -1, cv2.LINE_AA)
        cv2.ellipse(img, (cx, cy + int(2 * escala)), (body_w // 2, max(5, int(body_h * 0.24))), 0, 0, 180, (0, 0, 255), 2, cv2.LINE_AA)

        # --- Cúpula con reflejo ---
        cv2.ellipse(img, (cx, cy - int(body_h * 0.45)), (dome_w // 2, dome_h // 2), 0, 180, 360, (80, 120, 255), -1, cv2.LINE_AA)
        cv2.ellipse(img, (cx, cy - int(body_h * 0.45)), (dome_w // 2, dome_h // 2), 0, 180, 360, (50, 80, 255), 2, cv2.LINE_AA)
        cv2.ellipse(img, (cx - int(dome_w * 0.2), cy - int(body_h * 0.55)), (int(dome_w * 0.18), int(dome_h * 0.18)), 0, 0, 360, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    def _dibujar_astronauta_azul(self, img, escala=1.0):
        ...
        # --- Efecto de humo cuando el astronauta es pequeño (al fondo) ---
        if escala < 0.7:
            for i in range(3):
                alpha = 0.18 - i * 0.05
                smoke = img.copy()
                cv2.circle(smoke, (cx, cy + int(18 * escala)), int(12 * escala + 6 * i), (120, 120, 120), -1, cv2.LINE_AA)
                cv2.addWeighted(smoke, alpha, img, 1.0 - alpha, 0, img)
